...
import myWebClient from './my_web_client.jsx';
...
export default class Client {
    constructor() {
        ...
        // this.url = 'http://matt.siteview.com';//'http://matt.siteview.com:8065'; 192.168.9.40:8065
        // this.url = 'http://192.168.9.39:10080';//'http://mattermost.siteview.com:8065'; 192.168.9.40:8065
        // this.url = 'http://220.168.30.10:10080';//'http://mattermost.siteview.com:8065'; 192.168.9.40:8065
        this.urlVersion = '/api/v3'; // /api/v3
        this.defaultHeaders = {
    ...
    //     let cookie = cookieArr[i].replace(" ","");
    //     let oneCookie = cookie.split('=');
    //     cookieObj[oneCookie[0].replace(" ","")] = oneCookie[1].replace(" ","");
    //   }
    //   cookieObj[key] = value;
    //   //重新设置
    //   cookieArr = [];
    //   for(let name in cookieObj){
    //     cookieArr.push(''+name+'='+cookieObj[name]);
    //   }
    ...
    getInitialLoad(success, error) {
      ...
      this.token = this.token || localStorage.getItem(this.tokenName);
      // console.log(this.token);
      if (!this.token) {
        console.log('No Token -------------------', this.token);
        window.location.href = '/';
        return;
      }
      this.defaultHeaders[HEADER_TOKEN] = this.token;
        request.
                ...
                (data, res) => {
                    ...
                    if (res && res.header) {
                      // console.log(res.header);
                        ...
                        let cookie = JSON.parse(res.header["cookies"]);
    ...
    getFileInfosForPost(channelId, postId, success, error) {
      // console.log('getFileInfosForPost--------------');
        request.
