...
import VertxClient from 'client/vertx_client.jsx';
...
export function initialize() {
  ...
  // connUrl = "ws://192.168.9.40:80/api/v3/users/websocket";

  VertxClient.setEventCallback(handleEvent);
  // VertxClient.setFirstConnectCallback(handleFirstConnect);
  // VertxClient.setReconnectCallback(() => reconnect(false));
  // VertxClient.setMissedEventCallback(() => {
  //   if (global.window.mm_config.EnableDeveloper === 'true') {
  //     Client.logClientError('missed websocket event seq=' + VertxClient.eventSequence);
  //   }
  //   reconnect(false);
  // });
  // VertxClient.setCloseCallback(handleClose);
  VertxClient.initialize();
}
...
function handleClose(failCount) {
  if (failCount > MAX_WEBSOCKET_FAILS) {
    ErrorStore.storeLastError({
      message: Utils.localizeMessage('channel_loader.socketError', 'Please check connection, Mattermost unreachable. If issue persists, ask administrator to check WebSocket port.')
    });
  }
...
function handleTeamAddedEvent(msg) {
  Client.getTeam(msg.data.team_id, (team) => {
    AppDispatcher.handleServerAction({type: ActionTypes.RECEIVED_TEAM, team});

    Client.getMyTeamMembers((data) => {
      AppDispatcher.handleServerAction({type: ActionTypes.RECEIVED_MY_TEAM_MEMBERS, team_members: data});
...
function handleUserRemovedEvent(msg) {
  if (UserStore.getCurrentId() === msg.broadcast.user_id) {
    ...
    if (msg.data.remover_id !== msg.broadcast.user_id && msg.data.channel_id === ChannelStore.getCurrentId() && $('#removed_from_channel').length > 0) {
      var sentState = {};
...
function handleReactionAddedEvent(msg) {
  ...
  AppDispatcher.handleServerAction({type: ActionTypes.ADDED_REACTION, postId: reaction.post_id, reaction});
}

function handleReactionRemovedEvent(msg) {
  const reaction = JSON.parse(msg.data.reaction);

  AppDispatcher.handleServerAction({type: ActionTypes.REMOVED_REACTION, postId: reaction.post_id, reaction});
}
