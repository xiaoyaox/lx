...
export function emitChannelClickEvent(channel) {
    ...
    function switchToChannel(chan) {
        ...
        AsyncClient.getChannelMember(chan.id, UserStore.getCurrentId()).then(() => {
            // console.log('AsyncClient.getChannelStats --------');
            AsyncClient.getChannelStats(chan.id, true);
            // console.log('AsyncClient.viewChannel --------');
    ...
    // console.log('emitChannelClickEvent --------------');
    // console.log(channel);
    if (channel.fake) {
    ...
    Client.getInitialLoad(
            (data, res) => {
                ...
                if (data.user && data.user.id) {
                    VertxClient.setUserId(data.user.id);
                ...
                if (data.teams) {
                    VertxClient.setTeams(data.teams);
                    var teams = {};
...
export function redirectToModulesPage(){ //重定向到模块选择页面。
...
export function redirectUserToDefaultTeam() {
    ...
    function redirect(teamName, channelName) {
      // console.log('redirectUserToDefaultTeam,', 'teamName:', teamName, 'channelName:', channelName);
