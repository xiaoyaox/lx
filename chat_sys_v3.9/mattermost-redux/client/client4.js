...
export default class Client4 {
    ...
    getWebsocketUrl(){
      let connectionUrl =((this.url).indexOf(window.serverPort)!=-1)?this.url:(this.url+':'+window.serverPort);
    ...
    setLocalStorageToken(value) { //设置存储在localStorage里的token值。
      return localStorage.setItem(this.tokenName,value);
    }
    ...
    login = async (loginId, password, token = '', deviceId = '', ldapOnly = false) => {
        ...
        if(data[HEADER_TOKEN]){
          this.token = data[HEADER_TOKEN];
        }
        if (headers.has(HEADER_TOKEN)) {
            this.token = headers.get(HEADER_TOKEN);
        }
        this.setLocalStorageToken(this.token); //记录登录的token值。
    ...
    doFetchWithResponse = async (url, options) => {
        ...
        let temp_serverVersionId = '3.8.0.3.8.1.57b4d7994ff08bc96ec775053291eb1a.false';
        if (headers.has(HEADER_X_VERSION_ID)) {
            const serverVersion = headers.get(HEADER_X_VERSION_ID);
            if (serverVersion && this.serverVersion !== serverVersion) {
                this.serverVersion = serverVersion;
                EventEmitter.emit(General.CONFIG_CHANGED, serverVersion || temp_serverVersionId);
            }
        }
