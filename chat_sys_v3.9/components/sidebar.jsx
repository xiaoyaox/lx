...
import ShowBlacklistUsers from './blacklist/show_blacklist_users.jsx';
import AddUsersToBlacklist from './blacklist/add_users_to_blacklist.jsx';
...
export default class Sidebar extends React.Component {
    constructor(props) {
        ...
        //blacklist modal
        this.showMoreBlacklistChannelsModal = this.showMoreBlacklistChannelsModal.bind(this);
        this.hideMoreBlacklistChannelsModal = this.hideMoreBlacklistChannelsModal.bind(this);
        this.hideAddUsersToBlacklistModal = this.hideAddUsersToBlacklistModal.bind(this);
        this.showAddUsersToBlacklistModal = this.showAddUsersToBlacklistModal.bind(this);
        ...
        state.showBlacklistChannelsModal = false;
        state.showAddUsersToBlacklistModal = false;
    ...
    // blacklist modal
    showMoreBlacklistChannelsModal() {
        this.setState({showBlacklistChannelsModal: true});
    }

    hideMoreBlacklistChannelsModal() {
        this.setState({showBlacklistChannelsModal: false});
    }

    // add users to blacklist modal
    showAddUsersToBlacklistModal() {
        this.setState({showAddUsersToBlacklistModal: true});
    }

    hideAddUsersToBlacklistModal() {
        this.setState({showAddUsersToBlacklistModal: false});
    }
    ...
    render() {
        ...
        const blacklistMessageItems = this.state.directAndGroupChannels.map((channel, index, arr) => {
            return this.createChannelElement(channel, index, arr, this.handleLeaveDirectChannel);
        });
        ...
        var blacklistMessageMore = (
            <li key='more'>
                <a
                    href='#'
                    onClick={(e) => {
                        e.preventDefault();
                        this.showMoreBlacklistChannelsModal();
                    }}
                >
                    <FormattedMessage
                        id='sidebar.moreElips'
        ...
        const createBlacklistTootlip = (
            <Tooltip id='add-blacklist-tooltip'>
                <FormattedMessage
        ...
        // create blacklist icon
        let createBlacklistIcon = (
            <OverlayTrigger
                delayShow={500}
                placement='top'
                overlay={createBlacklistTootlip}
            >
                <a
                    className='add-channel-btn'
                    href='#'
                    onClick={this.showAddUsersToBlacklistModal}
                >
                    {'+'}
                </a>
            </OverlayTrigger>
        ...
        // blacklist modal
        let moreBlacklistChannelsModal;
        if (this.state.showBlacklistChannelsModal) {
            moreBlacklistChannelsModal = (
                <ShowBlacklistUsers
                    onModalDismissed={this.hideMoreBlacklistChannelsModal}
                    showAddBlacklistModal={this.showAddUsersToBlacklistModal}
                />
            );
        }

        // add users to blacklist modal
        let addUsersToBlacklistModal;
        if (this.state.showAddUsersToBlacklistModal) {
            addUsersToBlacklistModal = (
                <AddUsersToBlacklist
                    onHide={this.hideAddUsersToBlacklistModal}
                />
            );
        }
